...
contains
   ...
   function read_ens_stats(F_dirname_S,F_varname_S,F_level,F_hour,F_nmembers,F_nci,F_ni,F_nj,F_params,F_exavg,F_gss,F_eavg,F_evar,F_inflation) result(F_istat)
      ...
      fileid = 0
      F_istat = fnom(fileid,trim(filename_S),'RND+R/O+OLD',0)
      F_istat = min(fstouv(fileid,'RND'),F_istat)
      ...
      istat = fstluk(pakdata,key,ni1,nj1,nk1)
      ...
      datev = RMN_ANY_DATE
      key = fst_find(fileid,'eavg',datev,F_level,F_hour,RMN_ANY_I)
      istat = fstluk(F_eavg,key,ni1,nj1,nk1)

      datev = RMN_ANY_DATE
      key = fst_find(fileid,'evar',datev,F_level,F_hour,RMN_ANY_I)
      istat = fstluk(F_evar,key,ni1,nj1,nk1)
